...
@everywhere begin
    ...
    # Helper to pmap through hyperparams
    function get_hyperparams(index::Int)
        batch_ids, learning_rates = hyperparams()

        nlr = length(learning_rates)
        bi = div(index - 1, nlr) + 1
        li = mod(index - 1, nlr) + 1

        return batch_ids[bi], learning_rates[li]
    end
    ...
    function run_experiment_and_plot(experiment_idx; verbose=false)

        batch_id, lr = get_hyperparams(experiment_idx)
...
b, l = hyperparams()
experiment_ids = collect(1:length(b) * length(l))

println("Starting $(length(experiment_ids)) experiments...")
pmap(run_experiment_and_plot, experiment_ids)
println("All done.")
